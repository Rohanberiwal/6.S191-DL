...
class CustomMitoticDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        box = torch.tensor(box, dtype=torch.float32).unsqueeze(0)  # Add batch dimension
...
def visualize_predictions(images, targets, model, device, epoch):
    ...
    with torch.no_grad():
        ...
        for i in range(len(images)):
            img = images[i].cpu().permute(1, 2, 0).numpy()  # Change to HWC format
            ...
            # Plot the predicted boxes in blue
            pred_boxes = predictions[i]['boxes'].cpu().numpy()  # Adjust according to your model output
...
# Modify train loop to visualize and save predictions every epoch
for epoch in range(num_epochs):
    train_loss = train_one_epoch(model, train_loader, optimizer, device, scheduler)
    print(f"Epoch [{epoch + 1}/{num_epochs}], Training Loss: {train_loss:.4f}")
    if epoch % 1 == 0:
        visualize_predictions(images[:2], targets[:2], model, device, epoch)
