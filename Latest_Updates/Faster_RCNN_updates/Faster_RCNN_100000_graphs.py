...
def compute_statistics(augmented_data):
    bounding_boxes = np.array([data['bounding_box'] for data in augmented_data])
    mean_bbox = np.mean(bounding_boxes, axis=0)
    median_bbox = np.median(bounding_boxes, axis=0)
    mode_bbox = stats.mode(bounding_boxes, axis=0).mode[0]
    std_bbox = np.std(bounding_boxes, axis=0)

    return mean_bbox, median_bbox, mode_bbox, std_bbox


import numpy as np
import matplotlib.pyplot as plt

def plot_scatter(augmented_data, title):
    bounding_boxes = np.array([data['bounding_box'] for data in augmented_data])
...
def aggressive_normalize_dataset(augmented_data):
    bounding_boxes = np.array([data['bounding_box'] for data in augmented_data])
...
def aggressive_normalize_mitotic_data(mitotic_data):
    bounding_boxes = np.array([data['bounding_box'] for data in mitotic_data])
    if np.any(bounding_boxes[:, 2] <= 0) or np.any(bounding_boxes[:, 3] <= 0):
        raise ValueError("Bounding boxes have invalid dimensions (width or height <= 0)")
...
plot_scatter(augmented_data, title='Scatter Plot of Bounding Box Corners (Before Normalization)')

augmented_data = augment_mitotic_data(mitotic_data, num_augmentations=10000)
augmented_data = aggressive_normalize_dataset(augmented_data)
plot_hyperplane(augmented_data)

plot_scatter(augmented_data, title='Scatter Plot of Bounding Box Corners (After Normalization)')
mean_bbox, median_bbox, mode_bbox, std_bbox = compute_statistics(augmented_data)
print("Mean Bounding Box:", mean_bbox)
print("Median Bounding Box:", median_bbox)
print("Mode Bounding Box:", mode_bbox)
print("Standard Deviation Bounding Box:", std_bbox)
import torch
...
mean_orig, median_orig, mode_orig ,  std_bbox_ori = compute_statistics(mitotic_data)
print("Mean Bounding Box:", mean_orig)
print("Median Bounding Box:", median_orig)
print("Mode Bounding Box:", mode_orig)
print("Standard Deviation Bounding Box:", std_bbox_ori)



plot_scatter(mitotic_data, title ="Mitotic data with 188 patch ")
mitotis =  aggressive_normalize_mitotic_data(mitotic_data)


mean_bbox, median_bbox, mode_bbox, std_bbox = compute_statistics(mitotis)
print("Mean Bounding Box:", mean_bbox)
print("Median Bounding Box:", median_bbox)
print("Mode Bounding Box:", mode_bbox)
print("Standard Deviation Bounding Box:", std_bbox)

plot_scatter(mitotis, title ="Mitotic data with 188 patch after aggressive normaliztion")


train_dataset = CustomMitoticDataset(mitotis, transforms=get_transform(train=True))
...
output_dir = "/content/output_predictions"
os.makedirs(output_dir, exist_ok=True)

def visualize_predictions(images, targets, model, device, epoch):
    model.eval()
    with torch.no_grad():
        ...
        for i in range(len(images)):
            ...
            for box in pred_boxes:
                x_min, y_min, x_max, y_max = box
                plt.gca().add_patch(plt.Rectangle((x_min, y_min), x_max - x_min, y_max - y_min,
                                                   fill=False, color='blue', linewidth=2, label='Prediction'))

            plt.axis('off')
            plt.title(f'Epoch: {epoch + 1}')
            plt.savefig(os.path.join(output_dir, f'epoch_{epoch + 1}_img_{i + 1}.png'))
            plt.close()
...
print("Phase 2 of the traning and testing on the augmented data set ")
from torch.utils.data import random_split, DataLoader

augmented_dataset = AugmentedDataset(augmented_data, transform=transform)
train_size = int(0.8 * len(augmented_dataset))
val_size = len(augmented_dataset) - train_size
train_data, val_data = random_split(augmented_dataset, [train_size, val_size])

train_loader = DataLoader(train_dataset, batch_size=1000, shuffle=True, collate_fn=collate_fn, num_workers=2)
val_loader = DataLoader(val_data, batch_size=1000, shuffle=False, collate_fn=collate_fn, num_workers=2)
