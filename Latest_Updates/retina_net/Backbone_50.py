...
def rescale_bboxes(bboxes, crop_position, crop_size, full_image_size):
    w_ratio = full_image_size[1] / crop_size[1]
    h_ratio = full_image_size[0] / crop_size[0]

    bboxes_rescaled = []
    for box in bboxes:
        x_min, y_min, x_max, y_max = box
        x_min = x_min * w_ratio + crop_position[0]
        y_min = y_min * h_ratio + crop_position[1]
        x_max = x_max * w_ratio + crop_position[0]
        y_max = y_max * h_ratio + crop_position[1]
        bboxes_rescaled.append([x_min, y_min, x_max, y_max])
    
    return torch.tensor(bboxes_rescaled)
...
for img_path in testerfile:
    ...
    crop_size = (256, 256)
    crop_position = (100, 150)

    resized_image = cv2.resize(image_rgb, (800, 800))
    detections = run_inference(model, resized_image)
    ...
    for i in detections:
        ...
        if mitotic_boxes.size(0) > 0:
            mitotic_boxes_rescaled = rescale_bboxes(mitotic_boxes, crop_position, crop_size, original_size)
            print("Mitotic Boxes (Threshold > 0.5):", mitotic_boxes_rescaled)
            print("Mitotic Scores (Threshold > 0.5):", mitotic_scores)
            plot_mitotic_detections(image_rgb, mitotic_boxes_rescaled, mitotic_scores, threshold)
