...
def get_dicts(standard_dict_mitotic, standard_dict_non_mitotic, split_ratio=0.7):
    dataset_dicts = []
    ...
    non_mitotic_dicts = []
    for idx, (image_path, boxes) in enumerate(standard_dict_non_mitotic.items(), start=len(mitotic_dicts)):
        record = {}
        ...
        for box in boxes:
            ...
            obj = {
                "bbox": [x, y, x + w, y + h],
                "bbox_mode": BoxMode.XYXY_ABS,
                "category_id": 0,  # Non-mitotic class
            }
            objs.append(obj)
        record["annotations"] = objs
        non_mitotic_dicts.append(record)

    # Combine and shuffle the dataset
    combined_dataset = mitotic_dicts + non_mitotic_dicts
    random.shuffle(combined_dataset)

    # Split the dataset into train and validation
    train_size = int(len(combined_dataset) * split_ratio)
    train_dataset = combined_dataset[:train_size]
    val_dataset = combined_dataset[train_size:]

    return train_dataset, val_dataset

train_dataset, val_dataset = get_dicts(standard_dict_mitotic, standard_dict_non_mitotic)
#DatasetCatalog.register("mitotic_non_mitotic_train", lambda: train_dataset)
#DatasetCatalog.register("mitotic_non_mitotic_val", lambda: val_dataset)
MetadataCatalog.get("mitotic_non_mitotic_train").set(thing_classes=["non-mitotic", "mitotic"])
MetadataCatalog.get("mitotic_non_mitotic_val").set(thing_classes=["non-mitotic", "mitotic"])
...
def setup_cfg():
    try :
        ...
        cfg.DATASETS.TRAIN = ("mitotic_non_mitotic_train",)
        cfg.DATASETS.TEST = ("mitotic_non_mitotic_val",)
        ...
        cfg.MODEL.ROI_HEADS.NUM_CLASSES = 2  # Number of classes (mitotic and non-mitotic)
...
print("Evrrhting doen perfectly")
