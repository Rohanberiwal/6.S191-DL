...
print("Second phase for the train and val")
...
def process_images_with_bboxes(standard_dict):
    ...
    for img_path, bboxes in standard_dict.items():
        ...
        for i, bbox in enumerate(bboxes):
            ...
            plt.imshow(img_copy)
            plt.title(f"Bounding Box {i+1} - Image: {img_filename}")
            plt.axis('off')
            plt.show()
...
def inspect_data_loader_new(data_loader, num_batches=1):
    for batch_idx, (images, targets) in enumerate(data_loader):
        if batch_idx >= num_batches:
            break
...
def train_one_epoch_new(model, data_loader, optimizer, device, scheduler=None):
    ...
    loss_list.append(total_loss_value)  # Append the scalar loss value
...
for epoch in range(num_epochs):
    ...
    validate_k_fold(model, val_loader_new ,   device, val_loss, k_folds=10)

plot_train_loss_graph(loss_list, len(loss_list))  # Plot training loss
plot_val_loss_graph(val_loss, len(val_loss))    # Plot validation loss


model_save_path = '/content/retiannet_model.pth'
...
def plot_mitotic_detections(image, boxes, scores, threshold=0.5):
    ...
    # Set axis to 'off' to remove any extra space and ticks
    ax.axis('off')
    ...
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)  # Remove any whitespace around the plot
    plt.show()

# Assuming the 'model' and 'testerfile' are predefined, and 'device' is set to 'cuda' or 'cpu'
class_names = ['Mitotic']
threshold = 0.70

for img_path in testerfile:
    image = cv2.imread(img_path)
    original_size = image.shape[:2]  # Get the original image size (height, width)
    ...
    if mitotic_boxes.size(0) > 0:
        ...
        print("Mitotic Boxes (Threshold > 0.70):", mitotic_boxes_converted)
        print("Mitotic Scores (Threshold > 0.70):", mitotic_scores)
