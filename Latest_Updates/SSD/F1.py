...
def validate(model, val_loader, device):
    model.train()  # Set the model to training mode (for loss computation)
    total_val_loss = 0.0
    num_batches = len(val_loader)
    
    with torch.no_grad():  # Disable gradient calculation for validation
        for images, targets in val_loader:
            images = [image.to(device) for image in images]  # Move images to the right device
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]  # Move targets to device

            # Forward pass through the model (computes the loss)
            loss_dict = model(images, targets)  # The model will compute the loss for each batch
            
            # Sum the losses (classification + bbox_regression) - the model handles it
            total_loss = sum(loss for loss in loss_dict.values())
            total_val_loss += total_loss.item()  # Sum the loss across all batches
...
# Assume num_epochs is defined somewhere in your training loop
num_epochs = 80 
...
for epoch in range(num_epochs):
    ...
    output_var =  validate(model, val_loader_new, device)
