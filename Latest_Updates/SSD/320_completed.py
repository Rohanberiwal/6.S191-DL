...
def calculate_metrics(predictions, targets, iou_thresholds=(0.5, 0.75, 0.95)):
    ...
    print(f"Precision at IoU {iou_thresh}: {precision_iou:.4f}")
    print(f"Recall at IoU {iou_thresh}: {recall_iou:.4f}")
    print(f"mAP at IoU {iou_thresh}: {mAP_iou:.4f}")
    print('-' * 40)
...
def validate_and_calculate_metrics(model, val_data_loader, device, k_folds=10):
    ...
    for fold in range(k_folds):
        ...
        print(calculate_metrics(all_preds, all_targets))
...
for epoch in range(num_epochs):
    ...
    validate_k_fold(model, val_loader_new ,   device, val_loss, k_folds=5)
    validate_and_calculate_metrics(model, val_loader, device)
