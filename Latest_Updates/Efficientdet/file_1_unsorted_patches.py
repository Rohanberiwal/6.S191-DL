...
mitotic_annotation_file = '/content/mitotic.json'
...
class CustomMitoticDataset(Dataset):
    def __init__(self, mitotic_data):
...
def print_mitotic_data(dataset):
  for idx in range(len(dataset)):
      image, target = dataset[idx]
      print(f"Data #{idx + 1}:")
      print(f"  Image Shape: {image.shape}")
      print(f"  Bounding Box: {target['boxes']}")
      print(f"  Label: {target['labels']}")
      print("-" * 40)

total_size = len(mitotic_data)
train_size = int(total_size * 1)
val_size = total_size - train_size
train_dataset = CustomMitoticDataset(mitotic_data[:train_size])
val_dataset = CustomMitoticDataset(mitotic_data[train_size:])
...
print("Len of the train is ", len(train_dataset))
print("Len of the val is ", len(val_dataset))
...
def prepare_bboxes_data(updated_dict):
    for image_path, bboxes in updated_dict.items():
        for bbox in bboxes:
...
processed_data = prepare_bboxes_data(updated_dict)
...
def process_images_with_bboxes(standard_dict):
    dataset_list = []
    ...
    for img_path, bboxes in standard_dict.items():
        ...
        for i, bbox in enumerate(bboxes):
            ...
            label = 1
            bbox_label = (x_min, y_min, x_max, y_max)
...
output_list = process_images_with_bboxes(standard_dict_mitotic)
...
class New_CustomMitoticDataset(Dataset):
    def __init__(self, mitotic_data):
        self.mitotic_data = mitotic_data
...
val_size = total_size - train_size
train_dataset_new = New_CustomMitoticDataset(output_list[:train_size])
val_dataset_new = New_CustomMitoticDataset(output_list[train_size:])
...
trainer_loader_new = DataLoader(train_dataset_new, batch_size=2, shuffle=True, num_workers=2, collate_fn=lambda x: tuple(zip(*x)))
val_loader_new = DataLoader(val_dataset_new, batch_size=2, shuffle=False, num_workers=2, collate_fn=lambda x: tuple(zip(*x)))

def plot_image_with_bbox(image_tensor, bbox_tensor):
...
for images, targets in trainer_loader_new:
    image = images[0]
    bbox = targets[0]['boxes']
    plot_image_with_bbox(image, bbox)
    break
...
def convert_boxes_format_new(boxes):
    new_boxes = []
    for box in boxes:
        x, y, width, height = box
        x_min = x
...
def calculate_metrics(predictions, targets, iou_thresholds=(0.5, 0.75, 0.95)):
    precision = []
    recall = []
    mAP_50 = []
    mAP_50_95 = []
    ...
    for iou_thresh in iou_thresholds:
        precision_iou = precision_score(targets, predictions, average='macro')
        recall_iou = recall_score(targets, predictions, average='macro')
        mAP_iou = average_precision_score(targets, predictions)

        precision.append(precision_iou)
        recall.append(recall_iou)
        mAP_50.append(mAP_iou)

    return precision, recall, mAP_50, mAP_50_95

def validate_and_calculate_metrics(model, val_data_loader, device, k_folds=10):
    model.eval()
    ...
    for fold in range(k_folds):
        ...
        with torch.no_grad():
            for images, targets in val_data_loader:
                ...
                for target in targets:
                    boxes = target['boxes']
                    target['boxes'] = convert_boxes_format_new(boxes)

                loss_dict = model(images, targets)
                losses = sum(loss for loss in loss_dict.values())
                total_val_loss += losses.item()

                pred_boxes, pred_conf, pred_class = model.predict(images)
                target_boxes = [target['boxes'] for target in targets]
                all_preds.append(pred_boxes)
                all_targets.append(target_boxes)

        precision, recall, mAP_50, mAP_50_95 = calculate_metrics(all_preds, all_targets)
...
def train_one_epoch_new(model, data_loader, optimizer, device, scheduler=None):
    model.train()
    total_loss = 0.0
    for images, targets in data_loader:
        images = [image.to(device) for image in images]
        for target in targets:
            boxes = target['boxes']
            boxes = convert_boxes_format_new(boxes)
            target['boxes'] = boxes
            validate_boxes_new(boxes)
...
def validate_k_fold(model, val_data_loader, device, val_loss, k_folds=10):
    model.train()
    ...
    for fold in range(k_folds):
        ...
        average_val_loss = total_val_loss / num_batches if num_batches > 0 else 0.0
        val_loss.append(average_val_loss)

        print(f"Fold {fold + 1}/{k_folds} - Average Validation Loss: {average_val_loss}")



def plot_train_loss_graph(train_losses, num_epochs):
    plt.figure(figsize=(10, 6))
...
def plot_val_loss_graph(val_losses, num_epochs):
    plt.figure(figsize=(10, 6))
...
for epoch in range(num_epochs):
    epoch_output = train_one_epoch_new(model, trainer_loader_new, optimizer, device, scheduler=None)
    print(f"Epoch {epoch + 1}, Loss: {epoch_output}")
    validate_k_fold(model, val_loader_new ,   device, val_loss, k_folds=20)
    precision, recall, mAP_50, mAP_50_95 = validate_and_calculate_metrics(model, val_loader, device)
    print("--Epoch", epoch, "--")
    print("Precision:", precision)
    print("Recall:", recall)
    print("mAP@0.50:", mAP_50)
    print("mAP@0.50-0.95:", mAP_50_95)
    print()


plot_train_loss_graph(loss_list, len(loss_list))
plot_val_loss_graph(val_loss, len(val_loss))
...
for img_path in testerfile:
    ...
    if mitotic_boxes.size(0) > 0:
        ...
        print("Mitotic Boxes (Threshold > 0.62):", mitotic_boxes_converted)
        print("Mitotic Scores (Threshold > 0.62):", mitotic_scores)
